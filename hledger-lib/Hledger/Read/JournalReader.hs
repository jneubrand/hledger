...
bucketdirectivep = do
  ...
  acct <- modifiedaccountnamep True
...
transactionp = do
  ...
  let postingsBucketed = (
        postings ++ [posting { pdate=Nothing
                             ...
                             } | all hasAmount postings && isJust acc && length postings == 1])
  let sourcepos = (startpos, endpos)
...
tests_JournalReader = testGroup "JournalReader" [
  ...
  ,testGroup "transactionp" [
    ...
    ,testCase "unbalanced multi-leg transaction within bucketed context: not auto-balanced" $ do
      ep <- parseWithState nulljournal{jparsedefaultaccount=Just "b"} transactionp
        (T.unlines
          ["2009/1/1 x"
          ," a  -2"
